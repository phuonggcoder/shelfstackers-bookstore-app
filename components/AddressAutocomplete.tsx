...
const AddressAutocomplete: React.FC<AddressAutocompleteProps> = ({ onAddressSelect }) => {
  ...
  const handleProvinceSearchChange = (text: string) => {
    ...
    if (text.length >= 2) {
      setTimeout(() => {
        if (text === provinceSearchQuery) { // Only search if text hasn't changed
          loadProvinces(text);
        }
      }, 300);
  ...
  const handleWardSearchChange = (text: string) => {
    ...
    if (selectedProvince) {
      ...
      if (text.length >= 2) {
        setTimeout(() => {
          if (text === wardSearchQuery) { // Only search if text hasn't changed
  ...
  const renderWardItem = ({ item }: { item: Ward }) => (
    ...
    >
      ...
      <View style={styles.wardDetails}>
        ...
        {item.hasMerger && (
          <Text style={styles.mergerInfo}>📋 Có lịch sử sáp nhập</Text>
        )}
      </View>
      {item.oldUnits && item.oldUnits.length > 0 && (
        <Text style={styles.oldUnits}>
          Trước đây: {item.oldUnits.join(', ')}
        </Text>
      )}
    </TouchableOpacity>
  );
