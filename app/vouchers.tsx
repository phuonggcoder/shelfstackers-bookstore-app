...
const VoucherScreen = () => {
  ...
  // Move TABS inside component to use latest t()
  const TABS = [
    { key: 'all', label: t('all') },
        ...
        <View style={styles.voucherHeaderRow}>
          ...
          {expired ? (
            ...
            <View style={styles.voucherTimeRow}>
              <Text style={styles.voucherDaysLeft}>{t('days left', { count: daysLeft })}</Text>
              <Text style={styles.voucherValidUntil}>{t('valid until', { date: new Date(v.end_date).toLocaleDateString('en-GB') })}</Text>
            </View>
          ) : (
            <Text style={styles.voucherValidUntil}>{t('valid until', { date: new Date(v.end_date).toLocaleDateString('en-GB') })}</Text>
          )}
    ...
    <View style={styles.container}>
      ...
      <View style={styles.inputRow}>
        ...
        <TouchableOpacity style={styles.findBtn}>
          ...
          <Text style={styles.findBtnText}>{t('find more voucher')}</Text>
        </TouchableOpacity>
      </View>
      <FlatList
        data={vouchers}
        ...
        style={{ marginTop: 10 }}
        ListEmptyComponent={<Text style={{ textAlign: 'center', color: '#888', marginTop: 40 }}>{t('no voucher')}</Text>}
        refreshing={loading}
