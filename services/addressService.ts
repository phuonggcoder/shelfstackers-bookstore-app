...
class AddressService {
  ...
  static async getWardsByProvince(provinceCode: string): Promise<Ward[]> {
    try {
      const response = await axios.get(`${BASE_URL}/api/address/wards`, {
  ...
  static async searchAddress(query: string): Promise<any[]> {
    try {
      const response = await axios.get(`${BASE_URL}/api/address/search`, {
        params: { q: query }
      });
      if (response.data && Array.isArray(response.data.data)) {
