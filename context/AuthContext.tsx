...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signIn = async (userData: AuthResponse) => {
    try {
      ...
      // Lấy lại user từ API theo token
      const userRes = await authService.getMe(userData.token);
      ...
      const deviceId = Device.osBuildId || Device.modelId || Device.deviceName || 'unknown';
      syncFcmToken((userRes.user || userRes)._id, deviceId, userData.token);
      listenFcmTokenRefresh((userRes.user || userRes)._id, deviceId);
